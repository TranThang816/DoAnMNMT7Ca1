...
if (!$item) {
    echo "<script>alert('Không tìm thấy tin nhắn liên hệ!'); window.location.href = 'admin_lienhe.php';</script>";
    exit;
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Chi tiết Liên hệ #<?= htmlspecialchars($item['ID']) ?></title>
...
<div class="detail-container">
    <div class="detail-header">
        <h2>Chi tiết liên hệ #<?= htmlspecialchars($item['ID']) ?></h2>
    </div>
    <div class="detail-body">
        ...
        <div class="field"><span class="label">SĐT:</span> <span class="value"><?= htmlspecialchars($item['SDT'] ?? 'N/A') ?></span></div>
        <div class="field"><span class="label">Nội dung:</span> <div class="value" style="white-space: pre-wrap;"><?= nl2br(htmlspecialchars($item['NOIDUNGTN'])) ?></div></div>
        ...
        <?php endif; ?>
        <div class="actions">
            <a href="admin_lienhe.php" class="btn btn-back">← Quay lại</a>
            <a href="phan_hoi_lienhe.php?id=<?= intval($item['ID']) ?>" class="btn btn-reply">Phản hồi / Cập nhật</a>
        </div>
    </div>
</div>

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $trangthai = isset($_POST['trangthai']) ? trim($_POST['trangthai']) : 'Chưa xử lý';
    $phanhoi = isset($_POST['phanhoi']) ? trim($_POST['phanhoi']) : null;

    if ($id <= 0) {
        echo "<script>alert('ID không hợp lệ'); window.location.href = 'admin_lienhe.php';</script>";
        exit;
    }

    $ok = $model->updateLienHe($id, $trangthai, $phanhoi);
    if ($ok) {
        echo "<script>alert('Đã cập nhật tin nhắn liên hệ'); window.location.href = 'admin_lienhe.php';</script>";
    } else {
...
$item = $model->getLienHeById($id);
if (!$item) {
    echo "<script>alert('Không tìm thấy tin nhắn'); window.location.href = 'admin_lienhe.php';</script>";
    exit;
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Phản hồi Liên hệ #<?= htmlspecialchars($item['ID']) ?></title>
...
<div class="form-container">
    <h2>Phản hồi / Cập nhật liên hệ #<?= htmlspecialchars($item['ID']) ?></h2>
    <p><strong>Khách hàng:</strong> <?= htmlspecialchars($item['HOTEN']) ?> — <strong>Email:</strong> <?= htmlspecialchars($item['EMAIL']) ?> — <strong>SĐT:</strong> <?= htmlspecialchars($item['SDT'] ?? 'N/A') ?></p>
    <form method="POST" action="phan_hoi_lienhe.php">
        <input type="hidden" name="id" value="<?= intval($item['ID']) ?>">
        <div class="field">
            <label>Trạng thái</label>
            <select name="trangthai" required>
                <?php
                    $options = ['Chưa xử lý', 'Đã phản hồi'];
        ...
        <div class="field">
            <label>Phản hồi (ghi chú hoặc nội dung phản hồi)</label>
        ...
        <div class="actions">
            ...
            <a href="admin_lienhe.php" class="btn btn-back">Hủy</a>
        </div>
    </form>
</div>
